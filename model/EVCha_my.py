...
from model.peak import PeakZoomModel
...
class cheb_conv_withSAt(nn.Module):
    ...
    def forward(self, x, spatial_attention, adj_pa, adj_sim, dynamic_gat):
        '''
        ...
        for time_step in range(num_of_timesteps):
            ...
            for k in range(self.K):
                T_k = self.cheb_polynomials[k]  # (N,N)
                mask = self.mask[k]
                myspatial_attention = spatial_attention[:, k, :, :] + adj_sim.mul(mask)
                myspatial_attention = F.softmax(myspatial_attention, dim=1)
                T_k_with_at = T_k.mul(myspatial_attention)  # (N,N)*(N,N) = (N,N) 多行和为1, 按着列进行归一化
                theta_k = self.Theta[k]  # (in_channel, out_channel)
                rhs = T_k_with_at.permute(0, 2, 1).matmul(
            ...
            for k in range(self.K):
                T_k = self.cheb_polynomials[k]  # (N,N)
                mask = self.mask[k]
                myspatial_attention = spatial_attention[:, k, :, :] + dynamic_gat.squeeze(1).mul(mask)
                myspatial_attention = F.softmax(myspatial_attention, dim=1)
                T_k_with_at = T_k.mul(myspatial_attention)  # (N,N)*(N,N) = (N,N) 多行和为1, 按着列进行归一化
                theta_k = self.Theta[k]  # (in_channel, out_channel)
                rhs = T_k_with_at.permute(0, 2, 1).matmul(
...
class GTU(nn.Module):
    def __init__(self, in_channels, time_strides, kernel_size):
        ...
        self.kernel_size = kernel_size  ###新加的
    ...
    def forward(self, x):
        ...
        x_in = x[:, :, :, self.kernel_size - 1:] #g改
        x_gtu =(x_q+x_in)*torch.sigmoid(x_p)#改,仿照原始gated cnn的代码改的
        return x_gtu


class EVCha_block(nn.Module):

    def __init__(self, DEVICE, num_of_d, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
                 cheb_polynomials, adj_pa, adj_sim, num_of_vertices, num_of_timesteps, d_model, d_k, d_v,
                 n_heads):
        super(EVCha_block, self).__init__()
        ...
        self.adj_sim = torch.FloatTensor(adj_sim).cuda()
        ...
        # self.fcmy = nn.Sequential(
        #     nn.Linear(num_of_timesteps-2, num_of_timesteps),
        #     nn.Dropout(0.05),
        # )
        self.ln = nn.LayerNorm(nb_time_filter)
        # wsw：定义1D卷积层，将1024个输入通道转换为512个输出通道
        self.conv_layer = nn.Conv1d(in_channels=1024, out_channels=512, kernel_size=1, stride=1, padding=0)



    def forward(self, x, res_att, dynamic_gat, dynamic_gcn,peak_xi):
        '''
        :param x: (Batch_size, N, F_in, T)
        ...
        if num_of_features == 1:
            ...
            TEmx_peak = self.EmbedT(peak_xi, batch_size)  # B,F,T,N
        else:
            TEmx = x.permute(0, 2, 3, 1)
            TEmx_peak = x.permute(0, 2, 3, 1)
        TATout, re_At = self.TAt(TEmx, TEmx, TEmx, None, res_att)  # B,F,T,N; B,F,Ht,T,T
        TATout_peak, re_At_peak = self.TAt(TEmx_peak, TEmx_peak, TEmx_peak, None, res_att)  # B,F,T,N; B,F,Ht,T,T

        x_TAt = self.pre_conv(TATout.permute(0, 2, 3, 1))[:, :, :, -1].permute(0, 2, 1)  # B,N,d_model
        x_TAt_peak = self.pre_conv(TATout_peak.permute(0, 2, 3, 1))[:, :, :, -1].permute(0, 2, 1)  # B,N,d_model

        x_combined = torch.cat((x_TAt, x_TAt_peak), dim=-1)  # x_combined 的形状将是 (8, 12, 1024)
        # 调整维度顺序以符合 Conv1d 的输入要求
        x_combined = x_combined.permute(0, 2, 1)  # 将形状调整为 (8, 1024, 12)
        # 应用1D卷积层
        x_output = self.conv_layer(x_combined)  # 应用卷积层，输出形状为 (8, 512, 12)
        # 调整回原来的形状
        x_TAt_conbined = x_output.permute(0, 2, 1)  # 将形状调整为 (8, 12, 512)



        # SAt
        SEmx_TAt = self.EmbedS(x_TAt_conbined, batch_size)  # B,N,d_model
        SEmx_TAt = self.dropout(SEmx_TAt)  # B,N,d_model
        STAt = self.SAt(SEmx_TAt, SEmx_TAt, None)  # B,Hs,N,N

        # graph convolution in spatial dim
        spatial_gcn = self.cheb_conv_SAt(x, STAt, self.adj_pa, self.adj_sim, dynamic_gat)  # B,N,F,T
...
class EVCha_submodule(nn.Module):
    def __init__(self, DEVICE, num_of_d, nb_block, in_channels, K, nb_chev_filter, nb_time_filter, time_strides,
                 cheb_polynomials_list, adj_pa_list, adj_sim_list, num_for_predict, len_input, num_of_vertices, d_model, d_k,
                 d_v, n_heads):
        '''
        :param nb_block:
        :param in_channels:
        ...
        :param time_strides:
        :param cheb_polynomials_list: list of cheb_polynomials for each dataset
        :param adj_pa_list: list of adjacency matrices for each dataset
        :param adj_sim_list: list of similarity matrices for each dataset
        :param num_for_predict:
        '''

        super(EVCha_submodule, self).__init__()

        self.BlockLists = nn.ModuleList()

        for cheb_polynomials, adj_pa, adj_sim in zip(cheb_polynomials_list, adj_pa_list, adj_sim_list):
            block_list = nn.ModuleList([EVCha_block(DEVICE, num_of_d, in_channels, K,
                                                      nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials,
                                                      adj_pa, adj_sim, num_of_vertices, len_input, d_model,
                                                      d_k, d_v, n_heads)])

            block_list.extend([EVCha_block(DEVICE, num_of_d * nb_time_filter, nb_chev_filter, K,
                                             nb_chev_filter, nb_time_filter, 1, cheb_polynomials,
                                             adj_pa, adj_sim, num_of_vertices, len_input // time_strides,
                                             d_model, d_k, d_v, n_heads) for _ in range(nb_block - 1)])

            self.BlockLists.append(block_list)

        self.final_conv = nn.ModuleList([nn.Conv2d(int((len_input / time_strides) * nb_block), 128, kernel_size=(1, nb_time_filter)) for _ in range(len(cheb_polynomials_list))])
        self.final_fc = nn.ModuleList([nn.Linear(128, num_for_predict) for _ in range(len(cheb_polynomials_list))])
        self.DEVICE = DEVICE

        # wsw初始化 PeakZoomModel
        self.peak_zoom_model = PeakZoomModel().to(DEVICE)

        self.to(DEVICE)

    def forward(self, x, dynamic_gat_list, dynamic_gcn_list):
        '''
        :param x: list of (B, N_nodes, F_in, T_in) for each dataset
        :param dynamic_gat_list: list of dynamic GAT inputs for each dataset
        :param dynamic_gcn_list: list of dynamic GCN inputs for each dataset
        :return: list of (B, N_nodes, T_out) for each dataset
        '''

        # 获取峰值注意力处理后的数据
        peak_xi, threshold_prediction = self.peak_zoom_model(x[0], x[1])

        outputs = []
        for i, (block_list, dynamic_gat, dynamic_gcn) in enumerate(zip(self.BlockLists, dynamic_gat_list, dynamic_gcn_list)):
            x_i = x[i]
            res_att = 0
            need_concat = []
            for block in block_list:
                x_i, res_att = block(x_i, res_att, dynamic_gat, dynamic_gcn,peak_xi)
                need_concat.append(x_i)

            final_x = torch.cat(need_concat, dim=-1)
            output1 = self.final_conv[i](final_x.permute(0, 3, 1, 2))[:, :, :, -1].permute(0, 2, 1)
            output = self.final_fc[i](output1)
...
def make_model(DEVICE, num_of_d, nb_block, in_channels, K,
               ...
               adj_mx_taxi, adj_pa_taxi, adj_sim_taxi,
               adj_mx_private, adj_pa_private, adj_sim_private,
               num_for_predict, len_input, num_of_vertices, d_model, d_k, d_v, n_heads):
    '''
    :param DEVICE:
    :param nb_block:
    :param in_channels:
    ...
    def prepare_graph(adj_mx, adj_pa, adj_sim):
        L_tilde = scaled_Laplacian(adj_mx)
        cheb_polynomials = [torch.from_numpy(i).type(torch.FloatTensor).to(DEVICE) for i in cheb_polynomial(L_tilde, K)]
        return cheb_polynomials, adj_pa, adj_sim

    cheb_polynomials_taxi, adj_pa_taxi, adj_sim_taxi = prepare_graph(adj_mx_taxi, adj_pa_taxi, adj_sim_taxi)
    cheb_polynomials_private, adj_pa_private, adj_sim_private = prepare_graph(adj_mx_private, adj_pa_private, adj_sim_private)

    model = EVCha_submodule(DEVICE, num_of_d, nb_block, in_channels,
                              K, nb_chev_filter, nb_time_filter, time_strides,
                              [cheb_polynomials_taxi, cheb_polynomials_private],
                              [adj_pa_taxi,  adj_pa_private],
                              [adj_sim_taxi, adj_sim_private],
                              num_for_predict, len_input, num_of_vertices, d_model, d_k, d_v, n_heads)
